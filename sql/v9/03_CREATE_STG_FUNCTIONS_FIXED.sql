-- ============================================================================
-- V9 ANALYTICS: CREATE STG FUNCTIONS (ИСПРАВЛЕННАЯ ВЕРСИЯ)
-- ============================================================================
-- Цель: ETL функции с правильными именами колонок из raw схемы
-- Дата: 22 октября 2025 (исправлено после анализа raw схемы)
-- ============================================================================

-- ФУНКЦИЯ 1: refresh_stg_crm_events()
-- Исправлено: id_type вместо id, types_descr вместо name
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    ns.date_time::date as event_day,  -- FIXED: use date_time::date instead of days
    ns.type as event_type_id,
    nt.types_descr as event_type_name,  -- FIXED
    ...
    CASE
      WHEN ns.dogovor = 1 THEN dc.total_cost_of_the_contract  -- FIXED
      ELSE NULL
    END as contract_amount,

    CASE
      WHEN ns.dogovor = 1 THEN dc.currentdate  -- FIXED: use contract date from docs_clients
      ELSE NULL
  ...
  LEFT JOIN raw.itcrm_new_types nt ON ns.type = nt.id_type  -- FIXED
...
COMMENT ON FUNCTION stg.refresh_stg_crm_events() IS 'V9: Обновление stg.crm_events из raw.itcrm_new_source';

-- ============================================================================
-- ФУНКЦИЯ 2: refresh_stg_source_attribution()
-- Исправлено: id_event вместо event, id_form вместо form_id
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN er.id_event IS NOT NULL THEN 'event'  -- FIXED
      ELSE 'direct'
    ...
    CASE
      WHEN irr.id_request IS NOT NULL THEN 'internet_request'
      WHEN er.id_event IS NOT NULL THEN 'event'  -- FIXED
      WHEN nf.id_form IS NOT NULL THEN 'form'   -- FIXED
      ELSE 'direct'
    END as source_type,
    ...
    er.id_event as event_id,  -- FIXED
    nf.id_form as form_id,     -- FIXED
...
COMMENT ON FUNCTION stg.refresh_stg_source_attribution() IS 'V9: Обновление stg.source_attribution с парсингом JSONB code';

-- ============================================================================
-- ФУНКЦИЯ 3: refresh_stg_marketing_match()
-- Без изменений - структура fb/google таблиц правильная
-- ============================================================================
...
COMMENT ON FUNCTION stg.refresh_stg_marketing_match() IS 'V9: Обновление stg.marketing_match с связью CRM к FB/Google';
...
COMMENT ON FUNCTION stg.refresh_stg_fact_leads() IS 'V9: Обновление stg.fact_leads - единая таблица лидов';
...
COMMENT ON FUNCTION stg.refresh_stg_fact_contracts() IS 'V9: Обновление stg.fact_contracts - договоры с FIRST TOUCH атрибуцией';
...
COMMENT ON FUNCTION stg.refresh_all_stg_tables() IS 'V9: Обновить все таблицы STG (для ежедневного n8n workflow)';
