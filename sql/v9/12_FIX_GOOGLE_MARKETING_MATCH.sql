-- ============================================================================
-- FIX: Google Marketing Match - Campaign Name
-- ============================================================================
-- Problem: google_campaign_name is NULL for all Google matches
-- Root Cause: JOIN with google_ads_names doesn't work properly
-- Solution: Use google_ads_campaign_daily as source for campaign_name
...
BEGIN
  ...
  SELECT DISTINCT ON (attr.id_source)
    ...
    -- ========================================================================
    -- GOOGLE DETAILS (FIXED: Use google_ads_campaign_daily)
    -- ========================================================================
    google_clicks.campaign_id::text as google_campaign_id,
    google_campaign_daily.campaign_name as google_campaign_name,  -- FIXED
    google_clicks.ad_group_id::text as google_ad_group_id,
    google_names.ad_group_name as google_ad_group_name,

    -- ========================================================================
    -- UNIFIED FIELDS (FIXED: Use correct campaign_name)
    -- ========================================================================
    COALESCE(fb_leads.campaign_id, google_clicks.campaign_id::text) as campaign_id,
    COALESCE(fb_campaigns.name, google_campaign_daily.campaign_name) as campaign_name,  -- FIXED
  ...
  -- ========================================================================
  -- FACEBOOK MATCHING - 6 СПОСОБОВ (приоритет по точности)
  -- ========================================================================
  ...
  -- ========================================================================
  -- GOOGLE CAMPAIGN NAME (FIXED: Use google_ads_campaign_daily)
  -- ========================================================================
  ...
  -- Google ad group names (keep google_names for ad_group_name)
...
COMMENT ON FUNCTION stg.refresh_stg_marketing_match() IS 'V9: FIXED Google campaign_name linkage via google_ads_campaign_daily';

-- ============================================================================
-- TEST: Verify Google campaign_name fill rate
-- ============================================================================
SELECT
  'GOOGLE CAMPAIGN NAME FIX TEST' as test_name,
